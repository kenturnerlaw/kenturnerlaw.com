...
function showMessage(amount,direction,combined,netA,netB,basic,additions,method,explanation){
  ...
  $("method").textContent=method;$("explanation").textContent=explanation;
}
$("overnightsA").addEventListener("input",updateOvernights);
$("resetCalculator").addEventListener("click",()=>{$("calculator").reset();$("result").hidden=true;$("overnightsB").value=183;});
$("calculateSupport").addEventListener("click",()=>{
